...
def generateXML(data):
	...
	data = "".join(data)
	data = data.replace(' ALIGN="RIGHT"', '').replace(' ALIGN="CENTER"', '').replace(' CLASS="DYNACOLORTR1"', '').replace(' NOWRAP=""', '').replace(' STYLE="FONT-#SIZE: 15PX;', '').replace(' STYLE="FONT-SIZE: 15PX;"', '').replace(' WIDTH="5%"', '')
